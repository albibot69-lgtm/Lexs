...
Tab1:Section({
    Title = "Join discord for update Or Buy premium",
    TextXAlignment = "Left",
    TextSize = 17,
})
...
local Section = Tab0:Section({ 
    Title = "Blantant X5/X7 New Method",
    Icon = "fish",
    TextTransparency = 0.05,
    TextXAlignment = "Left",
    TextSize = 17,
...
local Button = Tab0:Button({
    Title = "X5 V2",
    Desc = "X5 New Update Hopefully Supports Ghostfin",
    Locked = false,
    Callback = function()
        loadstring(game:HttpGet("https://pastefy.app/A9L3xxQS/raw"))()
    end
})

local TabK = Window:Tab({
    Title = "Exclusive ( Kaitun )",
    Icon = "star",
})

local Button = TabK:Button({
    Title = "Kaitun",
    Desc = "you can afk using a newly created account without doing anything",
    Locked = false,
    Callback = function()
        loadstring(game:HttpGet("https://pandadevelopment.net/virtual/file/0b5d9a3c52985395"))()
    end
})
...
local function forceTPOnce(position)
    if not lastTPPos or (lastTPPos - position).Magnitude > 5 then
        lastTPPos = position
        for i = 1, 2 do
...
RunService.RenderStepped:Connect(function()
    if autoFloat and hrp then
        local pos = hrp.Position
        local targetY = Terrain.WaterLevel + floatOffset
...
local originalRenderAtPoint = VFXControllerModule.RenderAtPoint
local originalRenderInstance = VFXControllerModule.RenderInstance

-- Status toggle
local isVFXDisabled = false

-- TOGGLE (versi WindUI toggle kamu)
local removeSkinToggle = Tab7:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            -- Bersihkan semua efek aktif
            local cosmeticFolder = workspace:FindFirstChild("CosmeticFolder")
            if cosmeticFolder then
                pcall(function()
                    cosmeticFolder:ClearAllChildren()
                end)
            end
